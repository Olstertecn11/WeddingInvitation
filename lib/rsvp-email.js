...
function buildEmailHtml({ invitation, guest, attendanceStatus, roleDetails }) {
  const siteUrl = getSiteUrl();
  const invitationUrl = siteUrl && guest?.code ? `${siteUrl}/${guest.code}` : "";
            ...
            <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width: 640px; background: #fffaf3; border: 1px solid #dac7b9;">
              <tr>
                <td style="padding: 34px 28px 22px; text-align: center; background: #754337; color: #fffaf3;">
                  ...
                  <h1 style="margin: 16px 0 0; font-family: Georgia, 'Times New Roman', serif; font-size: 36px; line-height: 1.05; font-weight: 400;">Gracias, ${escapeHtml(guest.fullName || invitation.displayName)}</h1>
                </td>
              </tr>
              <tr>
                  ...
                  <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="background: #f3ebe2; border: 1px solid #e1d1c2;">
                    <tr>
                      <td style="padding: 18px; color: #685a50;">
                        <p style="margin: 0 0 8px;"><strong>Invitado:</strong> ${escapeHtml(guest.fullName)}</p>
                        <p style="margin: 0 0 8px;"><strong>Respuesta:</strong> ${attendanceLabel(attendanceStatus)}</p>
...
function buildEmailText({ invitation, guest, attendanceStatus, roleDetails }) {
  const siteUrl = getSiteUrl();
  const invitationUrl = siteUrl && guest?.code ? `${siteUrl}/${guest.code}` : "";
  const lines = [
    `Confirmación recibida - ${WEDDING_DETAILS.couple}`,
    "",
    `Gracias, ${guest.fullName || invitation.displayName}.`,
    `Respuesta: ${attendanceLabel(attendanceStatus)}`,
...
export async function sendRsvpConfirmationEmail({
  ...
  await transporter.sendMail({
    ...
    to,
    subject,
    html: buildEmailHtml({ invitation, guest, attendanceStatus, roleDetails }),
    text: buildEmailText({ invitation, guest, attendanceStatus, roleDetails }),
  });
